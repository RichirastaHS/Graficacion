...
export class CanvasLocal {
  ...
  protected binaryArray: any; 
  protected binaryIndex: number;
  protected verAnatomia: boolean;
  protected guardarUrl: string;
  ...
  marcadoresPosicion(x: number, y: number, color: string) {
  ...
  patronTemporizacionH(x: number, y: number, color: string) {
    for(let i = 0; i < 9; i++) {
      this.graphics.fillStyle = i%2 === 0 ? color : "#F3EFE0";
      this.dibujarPixel(x + i, y);
    }
  }
  franjasFormato(color: string, bitsFormato: string){
    let index = 0;
  //11101111//1000100
...
//dasdasdasdasda
  ...
  formatoInformacion(color: string){ 
    this.graphics.fillStyle = "#F3EFE0"; 
  ...
  numCaracteres(num: number, color: string) {
  ...
  urlToBinaryAscii(url: string): string {
    // Función interna para convertir un número a binario de 8 bits
    function to8BitBinary(n: number): string {
  ...
  lhxray(x: number, y:number, color:string, tam: number){
    let hi=x; 
    this.graphics.fillStyle = color;
  ...
  lvxray(x: number, y:number, color:string, tam: number){
    let hv=y; 
    for (let index = 0; index < tam; index++) {
  ...
  mcxray(x: number, y:number, color:string, area: number){
 ...
 pintarColumnaU(x: number, y:number, color: string, cantBits: number){
  let x2=x-1;
  let altura=y;
 ...
 pintarColumnaD(x: number, y:number, color: string, cantBits: number){
  let altura=y; 
  let x2=x-1;  
...
pintar1columna(x: number, y:number, color: string, cantBits: number){
  let altura=y; 
  for(let i=0; i<cantBits; i++){  
    this.binaryIndex --; 
